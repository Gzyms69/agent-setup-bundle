...
Based on fabric's `humanize` pattern. Apply these checks after the main audit to catch structural and tonal AI tells that survive word-level fixes.

### Buzzy overused words scan

Scan for these classic AI indicators that signal machine-generated text even when sentence structure is otherwise clean: **delve**, **testament**, **pivotal**, **beacon**, **landscape** (metaphor), **multi-faceted**, **myriad**, **realm**, **paradigm**, **embark**, **tapestry**, **robust**, **comprehensive**, **seamless**, **leverage**, **holistic**, **synergy**, **actionable**, **impactful**. Most of these overlap with the Tier 1 and Tier 2 word tables above — but the humanize check treats them as a cluster signal. Three or more in a single piece is a hard flag regardless of individual tier.

### Robotic sentence patterns

AI text falls into predictable sentence templates. Watch for:
- **Subject-verb-object monotony**: Every sentence follows the same grammatical shape. Break it up with questions, fragments, inversions, or subordinate-clause leads.
- **Parallel construction overuse**: Three consecutive sentences with the same structure ("X does A. Y does B. Z does C."). Vary the construction.
- **Declarative stacking**: Long runs of flat declarative sentences with no variation in mood (interrogative, imperative, exclamatory). Insert a question, a command, or a one-word sentence to break the pattern.
- **Passive-voice chains**: Three or more consecutive sentences in passive voice. Flip at least one to active.

### Cadence and rhythm variation

Sentence length is the strongest structural signal of AI text. Fix it deliberately:
- **Target**: No more than three consecutive sentences within five words of each other in length.
- **Mix**: Short punchy sentences (3-8 words) should appear at least once every 4-5 sentences. They hit harder after a long one.
- **Long sentences**: At least one sentence per section should run 25+ words — flowing, multi-clause, carrying the reader through a full thought before landing.
- **Fragments**: Use them. They work. Especially after setup.
- **Questions**: Break a declarative run with a direct question. Not a rhetorical one — a real one that the next sentence answers.
- **One-sentence paragraphs**: Deploy sparingly for emphasis. They force the reader to pause.

### Conversational tone without sacrificing accuracy

The goal is prose that reads like a knowledgeable person talking, not a textbook reciting. Techniques:
- Use contractions where they sound natural ("it's" not "it is", "don't" not "do not") — stiff formality is itself an AI tell.
- Allow first-person where appropriate ("I found," "in my experience") instead of detached third-person ("one might observe," "it has been noted").
- Permit colloquialisms that fit the register. "This breaks stuff" is fine in a technical blog. Don't sterilize voice in pursuit of correctness.
- Keep technical precision intact. Conversational tone means natural phrasing, not dumbing down. "The garbage collector runs every 30 seconds" is both casual and precise.

### Balanced-clause structure detection

AI produces suspiciously balanced sentences that hedge in both directions simultaneously:
- **Pattern**: "While X, also Y." / "Although A, however B." / "On one hand X, on the other hand Y."
- **The tell**: Both clauses are roughly the same length, both are vague, and the sentence takes no position. It reads as equivocation dressed up as nuance.
- **Fix**: Pick a side. If the concession matters, make it specific and brief, then give the main point the weight. "While X" should be short setup; "Y" should be the claim with evidence. If you can't pick a side, the sentence probably shouldn't exist.
- **Exception**: Genuine analytical comparison where both sides carry specific, different evidence. The problem is not the structure — it's when the structure contains no content.

---

## Clean Prose Protocol

Based on fabric's `clean_prose` pattern. Apply after the humanize pass to tighten prose without changing meaning. Every word must earn its place.

### Fluff elimination

Cut these categories on sight:
- **Redundant modifiers**: "completely eliminate" (eliminate already means completely), "absolutely essential" (essential is already absolute), "very unique" (unique is binary), "totally destroyed," "really important," "extremely critical."
- **Filler words**: "basically," "essentially," "actually" (when not contrasting), "literally" (when not literal), "really," "just" (when not temporal), "very," "quite," "rather."
- **Throat-clearing openers**: "It goes without saying that," "Needless to say," "As we all know," "It should be noted that," "The fact of the matter is." Delete the opener and start with the point.
- **Empty amplifiers**: "a wide range of" (use "many" or a number), "a large number of" (use "many" or a number), "in a manner that" (rewrite), "on a daily basis" (use "daily").

### Passive-to-active voice conversion

Passive voice is not always wrong — but AI defaults to it because it avoids naming the actor. Convert when:
- The actor is known and matters: "The report was generated by the system" becomes "The system generated the report."
- The passive hides responsibility: "Mistakes were made" becomes "We made mistakes" or name who did.
- The sentence is unnecessarily long because of passive construction.

**Keep passive when:**
- The actor is genuinely unknown or irrelevant: "The server was provisioned in us-east-1" (who provisioned it doesn't matter).
- The object is the topic of the paragraph and should lead the sentence.
- Scientific or technical convention requires it.

### Sentence tightening

Compress without losing meaning:
- **"The reason is because"** becomes "because."
- **"In spite of the fact that"** becomes "despite" or "although."
...
- **"With regard to"** / **"with respect to"** becomes "about" or "on."
- **"For the purpose of"** becomes "to" or "for."
- **"In close proximity to"** becomes "near."
- **Test**: If a shorter version says the same thing, use the shorter version. Always.

### Hedging language removal

These hedges weaken claims without adding accuracy. Cut or commit:
- **"Somewhat"** — Either the thing is true or quantify how much.
- **"Arguably"** — If you're arguing it, drop "arguably" and argue it. If you're not, don't invoke the word.
- **"It could be said that"** — By whom? Either attribute the claim or own it.
- **"To some extent"** — Quantify the extent or cut.
- **"In many ways"** — Name the ways or cut.
- **"It is generally accepted that"** — By whom? Cite or state directly.
- **"There is a sense in which"** — Name the sense or cut.
- **"One might argue"** — Are you arguing it? Then argue it. If not, cut.

**Carve-out**: Technical hedging that reflects genuine uncertainty is not fluff. "This approach may fail under high concurrency" is an honest caveat, not a hedge. The test: does removing the hedge change the truth of the statement? If yes, keep it. If no, cut it.

### Cross-reference: marketing contexts

When cleaning prose for marketing pages, landing pages, or conversion-focused content, apply this protocol in conjunction with the **marketing-copywriting** skill. The two skills complement each other:
- **Clean Prose Protocol** removes bloat and tightens structure.
- **marketing-copywriting** ensures the tightened prose still sells — clear value propositions, strong CTAs, benefit-led framing.

In marketing contexts, some repetition and emphasis that clean-prose would cut may be intentional for persuasion. Defer to marketing-copywriting guidance on: headline repetition strategy, CTA phrasing, and benefit stacking. Apply clean-prose to: body copy, feature descriptions, and any prose paragraph that isn't a headline or CTA.
